...
asyncio.run(main())
```
### launch的参数。launch传入的参数跟[Pypeteer](./03.Pypeteer.md#section_0703_01),命名有些差异
...
eval_on_selector<br/>eval_on_selector_all|在单个/所有符合选择器的节点执行js<br/>等效于[Pypeteer](./03.Pypeteer.md#section_0701_03)的querySelectorEval和querySelectorAllEval|page.eval_on_selector('h2', 'n => n.innerText')<br/>page.eval_on_selector_all('div.item a.name','nodes=>nodes.map(n=>n.href)')
on|事件监听(和[Pypeteer](./03.Pypeteer.md#section_0701_02)的一样)|page.on('response',lambda res: print(f"response:{res.status}: {res.url}"))
